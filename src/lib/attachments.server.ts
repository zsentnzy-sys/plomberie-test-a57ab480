...
export async function validateFiles(files: File[]): Promise<ValidatedFile[]> {
  ...
  for (const f of files) {
    ...
    if (f.size > MAX_SIZE) {
      throw new AttachmentValidationError(`Chaque photo doit faire moins de 5 Mo (« ${sanitizeDisplayName(f.name)} »).`)
    }
    total += f.size
...
): Promise<Array<{ id: string; storage_path: string; original_filename: string; mime_type: string; size_bytes: number }>> {
  const uploaded: string[] = []
  const insertedIds: string[] = []
  const rows: Array<{ id: string; storage_path: string; original_filename: string; mime_type: string; size_bytes: number }> = []
  try {
    ...
    if (insertedIds.length > 0) {
      await supabase.from('request_attachments').delete().in('id', insertedIds).then(() => undefined, () => undefined)
    }
    throw err
  }
}
